...
TestData_or_RealData = 0
if TestData_or_RealData
    % Sample data point
    p = [1 1; 2 3; 4 5; 6 6; 5 4; 6 7 ; 9 9; 12 15 ];
else
    % Real data: fish eye lens Theta vs R distortion data
    load BSplineData
end
...
title('B-Spline Test');
xlabel('X'); ylabel('Y');
plot(BSpline_l(:,1),BSpline_l(:,2),'rx');
hold on
b=plot(BSpline_l(:,1),BSpline_l(:,2),'r');
c=plot(Q(:,1),Q(:,2),'ms');
